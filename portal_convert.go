...
func (portal *Portal) convertDiscordVideoEmbed(ctx context.Context, intent *appservice.IntentAPI, embed *discordgo.MessageEmbed) *ConvertedMessage {
	...
	if isGIFV && gifvMode != "video" {
		...
		gifvFPS := portal.bridge.Config.Bridge.GIFV.Args.FPS
		gifvMaxWidth := portal.bridge.Config.Bridge.GIFV.Args.MaxWidth
		copyMeta = AttachmentMeta{
			Converter: func(data []byte) ([]byte, string, error) {
				return portal.bridge.convertGIFVToGIF(data, autoMaxSize, autoMode, gifvFPS, gifvMaxWidth)
			},
		}
	}
